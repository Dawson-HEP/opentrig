    ...
    match set_all {
        Some(true) => {
            ...
            if power_down_mode.is_some() {
                ...
                dac_manager.set_all_power_down_modes([power_down_mode.unwrap();24]).await.unwrap();
            } else {info!("invalid call to set_all_power_down_modes casting one value to 24")}
        },
        Some(false) => {
            let power_down_mode_option = from_idx_get_valid_type(2, inputs, fn_call::PowerDown_mode).await;
            if power_down_mode_option.is_some() {
                if let setting_values::PowerDown(power_down_modes) = power_down_mode_option.unwrap() {
                    info!("valid call to set_all_power_down_modes");
                    dac_manager.set_all_power_down_modes(power_down_modes).await.unwrap();
    ...
    match set_all {
        Some(true) => {
            ...
            if gain_mode.is_some() {
                ...
                dac_manager.set_all_gain_modes([gain_mode.unwrap();24]).await.unwrap();
    ...
    match set_all {
        Some(true) => {
            ...
            if vref.is_some() {
                ...
                dac_manager.set_all_vref_modes([vref.unwrap();24]).await.unwrap();
...
async fn call_set_all_voltages(inputs:&[u8], mut dac_manager:&mut DacManager<'static>) {
    ...
    match set_all {
        ...
        Some(false) => {
            ...
            if voltages.is_some() {
                info!("valid call to set_all_voltages");
                dac_manager.set_all_voltages(voltages.unwrap()).await.unwrap();
...
async fn call_set_power_down_mode(inputs:&[u8], mut dac_manager:&mut DacManager<'static>) {
    ...
    if dac_id.is_some() && channel.is_some() && mode.is_some() {
        ...
        dac_manager.set_power_down_mode(dac_id.unwrap(), channel.unwrap(), mode.unwrap()).await.unwrap();
...
async fn call_set_gain_mode(inputs:&[u8], mut dac_manager:&mut DacManager<'static>) {
    ...
    if dac_id.is_some() && channel.is_some() && mode.is_some() {
        ...
        dac_manager.set_gain_mode(dac_id.unwrap(), channel.unwrap(), mode.unwrap()).await.unwrap();
...
async fn call_set_vref_mode(inputs:&[u8], mut dac_manager:&mut DacManager<'static>) {
    ...
    if dac_id.is_some() && channel.is_some() && mode.is_some() {
        ...
        dac_manager.set_vref_mode(dac_id.unwrap(), channel.unwrap(), mode.unwrap()).await.unwrap();
...
async fn call_set_voltage(inputs:&[u8], mut dac_manager:&mut DacManager<'static>) {
    ...
    if dac_id.is_some() && channel.is_some() && voltage.is_some() {
        ...
        dac_manager.set_voltage(dac_id.unwrap(), channel.unwrap(), voltage.unwrap()).await.unwrap();
...
async fn from_idx_get_valid_type(idx:usize, inputs:&[u8], set_type:fn_call)
    ...
    let u8_items = [
         u8_1,  u8_2,  u8_3,  u8_4,  u8_5,  u8_6,
         u8_7,  u8_8,  u8_9, u8_10, u8_11, u8_12,
        u8_13, u8_14, u8_15, u8_16, u8_17, u8_18,
        u8_19, u8_20, u8_21, u8_22, u8_23, u8_24,
    ];
    ...
    match set_type {
        ...
        fn_call::PowerDown_mode => {
            ...
            let power_down_validity = u8_items.map(|power_down| match_power_down_mode(power_down).is_some());
        ...
        fn_call::Voltage => {Err::<bool, &str>("not implemented yet").unwrap();None}
    }
}
...
async fn from_idx_get_valid_voltages(idx:usize, inputs:&[u8]) -> Option<[u16;24]> {
    ...
    if  volt_1.is_some() &&  volt_2.is_some() &&  volt_3.is_some() &&  volt_4.is_some() &&
        volt_5.is_some() &&  volt_6.is_some() &&  volt_7.is_some() &&  volt_8.is_some() &&
        volt_9.is_some() && volt_10.is_some() && volt_11.is_some() && volt_12.is_some() &&
       volt_13.is_some() && volt_14.is_some() && volt_15.is_some() && volt_16.is_some() &&
       volt_17.is_some() && volt_18.is_some() && volt_19.is_some() && volt_20.is_some() &&
       volt_21.is_some() && volt_22.is_some() && volt_23.is_some() && volt_24.is_some() {
        Some(
            [ 
             volt_1.unwrap(),  volt_2.unwrap(),  volt_3.unwrap(),  volt_4.unwrap(),
             volt_5.unwrap(),  volt_6.unwrap(),  volt_7.unwrap(),  volt_8.unwrap(),
             volt_9.unwrap(), volt_10.unwrap(), volt_11.unwrap(), volt_12.unwrap(),
            volt_13.unwrap(), volt_14.unwrap(), volt_15.unwrap(), volt_16.unwrap(),
            volt_17.unwrap(), volt_18.unwrap(), volt_19.unwrap(), volt_20.unwrap(),
            volt_21.unwrap(), volt_22.unwrap(), volt_23.unwrap(), volt_24.unwrap(),
            ]
        )
       } else {
