...
async fn daq_sender_from_tlu(mut daq:DAQFpga<'static, embassy_rp::peripherals::SPI0>) {
    info!("awaiting TLU");
...
async fn handle_inputs_from_core_1(mut dac_manager:DacManager<'static>) {
...
async fn core0_task(mut dac_manager:DacManager<'static>,
    ...
    let daqs_from_tlu = daq_sender_from_tlu(daq);
    let input_handler = handle_inputs_from_core_1(dac_manager);
...
//
            ...
            // info!("Volume 0: {:?}", defmt::Debug2Format(&sd_volume0));
            // // Open the root directory (mutably borrows from the volume).
            // let mut sd_root_dir = sd_volume0.open_root_dir();
...
async fn handle_tlu_daqs(
    ...
    loop {
        ...
        let daq_sender = daq_sender_over_usb(&mut write_ep, &output);
        let sd_manager = sd_manager(&output);
